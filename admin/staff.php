...
<aside class="w-64 h-full basis-1/5"  aria-label="Sidebar">
   <div class="overflow-y-auto  py-4 px-3 h-full bg-gray-200 rounded dark:bg-gray-800">
      <ul class="space-y-2">
         ...
         <li>
            <a href="#" class="flex items-center items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-white bg-gray-100 dark:hover:bg-gray-700">
...
<p class="text-xl font-bold mb-6">Staff</p>
<div class="border-2 bg-gray-50 overflow-scroll shadow-lg rounded-xl p-4 border-black" id="example" style="height: 50%;">
         <div class="flex justify-between mb-6">
            <p class="text-lg">Receptionist</p>
         ...
         <div class="overflow-x-auto relative border-2 border-gray-400 sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500 overflow-y-scroll ">
               ...
               <?php
                     ...
                     for ($i = 1; $i <= $k; $i++) {
                        ...
                        $add = $ress[$i - 1]['staff_add'];
                        $cont = $ress[$i - 1]['staff_contact'];
                        $shift = $ress[$i - 1]['shift'];
                        $shiftt = 'Day';
                        if($shift == 1){$shiftt="Night";}
                        echo <<<qq
                  ...
                  <form method="POST">
                        ...
                        <input type="submit" name="delete" value="Delete" class="font-medium text-red-600 hover:underline">
                  </form>
                  </td>
                  </tr>
                  qq;
...
<div class="border-2 mt-10 bg-gray-50 overflow-scroll shadow-lg rounded-xl p-4 border-black" id="example" style="height: 50%;">
         <div class="flex justify-between mb-6">
            <p class="text-lg">Cleaning staff</p>
            <i class="fa-solid fa-pen-to-square hover:cursor-pointer" onclick="document.getElementById('c_edit-modal').classList.remove('hidden')"></i>
         </div>
         ...
         <table class="w-full text-sm text-left text-gray-500 overflow-y-scroll ">
               ...
               <?php
                  ...
                  <td class="py-4 px-6 text-left">
                  <form method="POST">
                        <input type="hidden" name="naa" value=$ii>
                        <input type="submit" name="beat" value="Delete" class="font-medium text-red-600 hover:underline">
                  </form>
                  </td>
                  </tr>
                  qq;
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  ...
                  <div class="py-6 px-6 lg:px-8">
                     ...
                     <form class="space-y-6" method="POST">
                        ...
                        <div>
                              <label for="cont" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Contact No.</label>
                              <input type="tel" name="cont" id="cont" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                        </div>

                        <div>
                              <label for="add" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Address</label>
                              <textarea name="add" id="add" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"   rows="5" required></textarea>
                        </div>

                        <div class="flex gap-2">
                           <div>
                              <input type="radio" value='0' name="radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                              <label for="default-radio-1" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Day</label>
                           </div>

                           <div>
                              <input type="radio" value='1' name="radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                              <label for="default-radio-1" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Night</label>
                           </div>
                        </div>
...
<div id="c_edit-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed flex items-center justify-center z-50 w-full bg-black bg-opacity-75 md:inset-0 h-modal md:h-full">
         <div class="relative p-4 w-full max-w-md h-full  md:h-auto">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" onclick="document.getElementById('c_edit-modal').classList.add('hidden')"  >
                     <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                     <span class="sr-only" >Close modal</span>
                  </button>
                  <div class="py-6 px-6 lg:px-8">
                     ...
                     <form class="space-y-6" method="POST">
                        ...
                        <button type="submit" name="cr_chng" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Save Changes</button>
...
if (isset($_POST['fr_chng'])) {
   ...
   $query = "INSERT INTO `receptionist`(`staff_name`, `staff_add`, `staff_contact`, `shift`) VALUES (?,?,?,?)";
   $values = [$frm_data['name'],$frm_data['add'],$frm_data['cont'],$frm_data['radio']];
...
if (isset($_POST['cr_chng'])) {
   $frm_data = filteration($_POST);
   $query = "INSERT INTO `cleaning_staff`(`staff_name`, `staff_add`, `staff_contact`, `shift`) VALUES (?,?,?,?)";
   $values = [$frm_data['name'],$frm_data['add'],$frm_data['cont'],$frm_data['radio']];
   $ress = updatess($query,$values);
...
if(isset($_REQUEST['delete'])){
   $nm = $_REQUEST['na'];
   $query = "DELETE FROM receptionist WHERE `staff_id` = ? ";
   $values = [$nm];
   $ress = updatess($query,$values);
   if($ress){
      echo "<meta http-equiv='refresh' content='0'>";
...
if(isset($_REQUEST['beat'])){
   $nm = $_REQUEST['naa'];
   $query = "DELETE FROM cleaning_staff WHERE `staff_id` = ? ";
   $values = [$nm];
   $ress = updatess($query,$values);
   if($ress){
      echo "<meta http-equiv='refresh' content='0'>";
