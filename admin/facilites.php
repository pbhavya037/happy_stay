...
<head>
  ...
  <title>Admin_Panel/Facilites</title>
...
<p class="text-xl font-bold mb-6">Features & Facilities</p>
            ...
            <table class="w-full text-sm text-left text-gray-500 overflow-y-scroll ">
               ...
               <tbody>
                  ...
                  <td class="py-4 px-6 text-left">
                     <form method="POST">
                        <input type="hidden" name="na" value='$nm'>
...
if (isset($_POST['fr_chng'])) {
   ...
   $query = "INSERT INTO features(`name`,`description`) VALUES (?,?)";
   $values = [$frm_data['name'],$frm_data['des']];
...
if(isset($_REQUEST['delete'])){
   $nm = $_REQUEST['na'];
   $query = "DELETE FROM features WHERE `name` = ? ";
   $values = [$nm];
   $ress = updatess($query,$values);
   if($ress){
      echo "<meta http-equiv='refresh' content='0'>";
