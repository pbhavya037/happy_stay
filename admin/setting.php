...
<head>
  ...
  <title>Admin_Panel/Setting</title>
...
<?php
   ...
   $ress = selectt($q,$v);
      if($ress){
         $title = $ress[0]['site_title'];
         $about = $ress[0]['site_about'];
      }
      else{
         ...
         if($res==1){
            $ress = selectt($q,$v);
               if($ress){
                  ...
                  alert('error','Unable to reset data');
               }
         }
...
if (isset($_POST['cont_chng'])) {
   $frm_data = filteration($_POST);
   $query = "UPDATE contact SET addres=?,map=?,phn1=?,phn2=?,email=? WHERE sr_no=?";
   $values = [$frm_data['address'],$frm_data['map'],$frm_data['phn1'],$frm_data['phn2'],$frm_data['mail'],"1"];
   $res = updatess($query,$values);
   if($res==1){
      $ress = selectt($q,$v);
         if($ress){
...
<aside class="w-64 h-full basis-1/5"  aria-label="Sidebar">
   <div class="overflow-y-auto  py-4 px-3 h-full bg-gray-200 rounded dark:bg-gray-800">
      <ul class="space-y-2">
         ...
         <li>
            <a href="#" class="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-white bg-gray-100 dark:hover:bg-gray-700">
...
<!-- Content -->
<div class=" basis-4/5 overflow-auto p-10" id="example">
   <div>
      <p class="text-xl font-bold mb-6">Settings</p>
      <div class="border-2 bg-gray-50 shadow-lg rounded-xl p-4 border-black">
         <div class="flex  justify-between">
            <p class="text-lg">General Settings</p>
            <i class="fa-solid fa-pen-to-square hover:cursor-pointer" onclick="document.getElementById('setting-modal').classList.remove('hidden')"></i>
         </div>
      ...
      <div class="border-2 bg-gray-50 shadow-lg mt-6 rounded-xl p-4 border-black">
         <div class="flex  justify-between">
            <p class="text-lg">Contact Settings</p>
            <i class="fa-solid fa-pen-to-square hover:cursor-pointer" onclick="document.getElementById('contact-modal').classList.remove('hidden')"></i>
         </div>
         <div class="flex justify-between px-10">
            <div>
               <h1 class="font-bold">Address</h1>
               <p><?php echo $address;?></p>
               <h1 class="font-bold">Google Map</h1>
               <p class="break-all "><?php echo $map;?></p>
            ...
            <iframe src="<?php echo $map;?>" loading="lazy" width="400" height="450"  frameborder="0"></iframe>
         </div>
      </div>
   </div>
<!-- modal setting -->
<!-- for general settings -->
<div id="setting-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed flex items-center justify-center z-50 w-full bg-black bg-opacity-75 md:inset-0 h-modal md:h-full">
         <div class="relative p-4 w-full max-w-md h-full  md:h-auto">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" onclick="document.getElementById('setting-modal').classList.add('hidden')"  >
                     <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                     <span class="sr-only" >Close modal</span>
                  </button>
                  <div class="py-6 px-6 lg:px-8">
                     <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Edit your changes</h3>
                     <form class="space-y-6" method="POST">
                        <div>
                              <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Site Title</label>
                        ...
                        <div>
                              <label for="about" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">About Us</label>
                              <textarea name="about" id="about" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"   rows="10" required></textarea>
                        </div>
                        <button type="submit" name="login" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Save Changes</button>
...
<!-- Contact settings -->
<div id="contact-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed flex items-center justify-center z-50 w-full bg-black bg-opacity-75 md:inset-0 h-modal md:h-full">
         <div class="relative p-4 w-full max-w-md h-full  md:h-auto">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" onclick="document.getElementById('contact-modal').classList.add('hidden')"  >
                     <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                     <span class="sr-only" >Close modal</span>
                  </button>
                  <div class="py-6 px-6 lg:px-8">
                     ...
                     <form class="space-y-6" method="POST">
                        <div>
                              ...
                              <textarea name="address" id="address" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"   rows="5" required></textarea>
                        </div>
                        <div>
                              <label for="map" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Google Map</label>
                              <input type="text" name="map" id="map" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                        </div>

                        <div>
                              <label for="mail" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Email</label>
                              <input type="email" name="mail" id="mail" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                        </div>

                        <div>
                              <label for="phn1" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Phone no. 1</label>
                              <input type="text" name="phn1" id="phn1" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                        </div>

                        <div>
                              <label for="phn2" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Phone no. 2</label>
                              <input type="text" name="phn2" id="phn2" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                        </div>
                        
                        <button type="submit" name="cont_chng" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Save Changes</button>
